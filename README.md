...
- OCR 전 이미지 미리보기 제공
- 전체/문장/단어 단위 TTS 재생 + 정지
- 음성 선택, 재생 속도 선택(영어 음성 우선)
- 받아쓰기 모드(단어/문장 선택, 1~3회 반복, 반복 간/문제 간 쉬는 시간, 진행률 표시)
- 받아쓰기 중 원문 숨기기 + 정답 보기 토글
- 틀린 문제만 다시 연습 UI 자리(추후 구현용)
- 샘플 문장 버튼, 초기화 버튼, 상태 메시지 영역
- OCR 텍스트 복사 버튼 + 저품질 OCR 힌트 메시지
- 텍스트 정리 버튼 + 감지된 문장/단어 개수 미리보기
- 상단 빠른 시작 가이드 + Phase 1 브라우저 TTS 품질 안내 문구
